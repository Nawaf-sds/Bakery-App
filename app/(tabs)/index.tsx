...
// const SERVER_URL = 'https://192.168.1.28:5000';
...
const getImageUrl = (source?: any) => {
  ...
  // تنظيف المسار من الفواصل المائلة المعكوسة (الويندوز)
  url = url.replace(/\\/g, '/');
    ...
    <View style={styles.promoSliderContainer}>
      <FlatList
        ...
        onMomentumScrollEnd={(event) => {
          setCurrentIndex(Math.round(event.nativeEvent.contentOffset.x / (width - 10)));
        }}
        contentContainerStyle={{ paddingHorizontal: 10 }}
      />
      <View style={styles.promoPagination}>
        {data.map((_, index) => (
          <View key={index} style={[styles.dot, { opacity: index === currentIndex ? 1 : 0.5 }]} />
        ))}
      </View>
    </View>
  );
...
export default function HomeScreen() {
  ...
  const formatDataForAlternatingLayout = (data: ProductType[]) => {
    ...
    while (i < data.length) {
      ...
      if (data[i + 1]) pair.push(data[i + 1]);
      if (pair.length > 0) {
        formattedData.push({ type: 'pair', items: pair, id: `pair_${i}` });
      }
      i += 2;
      if (i < data.length) {
        formattedData.push({ type: 'full', items: [data[i]], id: `full_${i}` });
        i += 1;
      }
    }
    return formattedData;
  ...
  if (error) {
    return (
      <View style={styles.errorContainer}>
        <Text style={styles.errorText}>حدث خطأ في الاتصال بالسيرفر.</Text>
      </View>
    );
  }
          ...
          renderItem={({ item }) => {
            if (item.type === 'pair') {
              return (
                <View style={styles.rowContainer}>
                  <View style={styles.halfWidth}>
                    <ProductItem item={item.items[0]} onToggleWishlist={toggleWishlist} />
                  </View>
                  {item.items[1] ? (
                    <View style={styles.halfWidth}>
                      <ProductItem item={item.items[1]} onToggleWishlist={toggleWishlist} />
                    </View>
                  ) : (
                    <View style={styles.halfWidth} />
                  )}
              ...
              const fullItem = item.items[0];
              const fullImgUri = getImageUrl(fullItem.imageUrls);
              return (
                <TouchableOpacity 
                  style={styles.fullCard} 
                  onPress={() => router.push(`/product-details/${fullItem._id}`)} 
                  activeOpacity={0.9}
                >
                  <Image 
